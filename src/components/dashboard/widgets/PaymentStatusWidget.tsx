...
import type { StatusData } from "./widgetDataProcessors";
...
const PaymentStatusWidget = ({ onRemove, isEditMode, data }: PaymentStatusWidgetProps) => {
  return (
      ...
      <CardContent className="h-56">
        {data.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={data} cx="50%" cy="50%" outerRadius={70} paddingAngle={2} dataKey="value" label={({ name, percent }) => `${(percent * 100).toFixed(0)}%`} labelLine={false}>
